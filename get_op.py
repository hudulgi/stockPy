...
targetTable = pd.read_csv(data_path + '\\target\\target_list.csv')
code_list = targetTable['code'].to_list()
code_list2 = code_list[:]

ignoreItem = pd.read_csv(data_path + '\\target\\ignore_item.csv')
...
print(resultDf)
resultDf.to_csv(data_path + '\\daily_data\\open_price\\op_' + nowDate.strftime("%y%m%d") +
                '.csv', index=False)
